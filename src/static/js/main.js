/* ============================================================
  ...
  11. CSRF 令牌注入（DEF-002）：所有写请求统一带上 X-CSRF-Token
   ============================================================ */
...
(function () {
    ...
    // 0. CSRF 令牌（DEF-002）
    // ============================================================
    // 后端会对所有写请求（POST/PUT/DELETE）校验 CSRF 令牌，缺失或过期一律 400。
    // 令牌由后端渲染进 <meta name="csrf-token">，这里统一读取并注入请求头，
    // 避免每个 fetch 调用点各写一遍 —— 将来新加一个 AJAX 却漏掉令牌，
    // 表现是「偶发保存失败」，排查成本很高。
    ...
    // 抽屉内容是通过 AJAX 塞进来的，里面带着服务端**重新签发**的令牌。
    // 顺手同步回 meta，让页面长时间开着时后续 AJAX 不会用到已过期的令牌。
    function syncCsrfToken(root) {
        var scope = root || document;
        var input = scope.querySelector('input[name="csrf_token"]');
        if (!input || !input.value) return;
        var meta = document.querySelector('meta[name="csrf-token"]');
        if (meta) meta.setAttribute('content', input.value);
    }
        ...
        open: function (url, afterLoad) {
            ...
            fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                ...
                .then(function (html) {
                    ...
                    syncCsrfToken(self.body);  // 用抽屉里新签发的令牌刷新 meta
        ...
        document.addEventListener('click', function (e) {
            ...
            if (msgItem) {
                ...
                if (msgItem.classList.contains('unread')) {
                    fetch('/messages/' + msgId + '/read', { method: 'POST', headers: csrfHeaders() })
            ...
            if (markAll) {
                fetch('/messages/read-all', { method: 'POST', headers: csrfHeaders() })
            ...
            if (remind) {
                ...
                fetch(rUrl, { method: 'POST', headers: csrfHeaders({ 'X-Requested-With': 'XMLHttpRequest' }) })
                    .then(function (r) { return r.json(); })
                    .then(function (data) {
