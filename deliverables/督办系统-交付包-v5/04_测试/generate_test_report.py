...
默认输出：仓库布局 → docs/测试报告/；交付包布局（位于 04_测试/）→ 本目录。

支持两种目录布局（自动识别，无需改代码）：
  1. 仓库布局  ：<项目根>/tests/generate_test_report.py
                 源码在 <项目根>/src/，测试套件在同目录 tests/
  2. 交付包布局：<交付包>/04_测试/generate_test_report.py
                 源码在 <交付包>/03_开发代码/src/，测试套件在 <交付包>/03_开发代码/tests/
"""
import os
import sys
import json
...
# ============================================================
# 目录自适应：先定位"含 src/ 的那个根"，再把需要的目录加入模块搜索路径
# ============================================================
TESTS_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_DIR = os.path.dirname(TESTS_DIR)
if not os.path.isdir(os.path.join(PROJECT_DIR, 'src')):
    # 交付包布局：本文件在 04_测试/ 下，源码根是同级的 03_开发代码/
    _cand = os.path.join(PROJECT_DIR, '03_开发代码')
...
import test_suite  # 项目测试套件（仓库：同目录 tests/；交付包：03_开发代码/tests/）
...
def main():
    # 默认输出：交付包布局（位于 04_测试/）→ 本目录；仓库布局 → 项目根/docs/测试报告
    if os.path.basename(TESTS_DIR) == "04_测试":
        default_out = TESTS_DIR
    else:
        default_out = os.path.join(PROJECT_DIR, "docs", "测试报告")
