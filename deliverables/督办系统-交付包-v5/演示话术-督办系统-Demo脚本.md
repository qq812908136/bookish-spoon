...
- **单一实例**：只运行一个程序实例；切换实例后先刷新或重新登录，否则改密码等会报 400「安全令牌无效」。
- **账号**：管理员 `admin` / `Supv#Admin2026`；负责人 `owner01`–`owner08` / `Supv#Owner2026`（内置演示库已灌 9 用户 / 45 任务 / 67 消息）。
- **AI 辅助（可选）**：若演示 AI，演示机需先装好 Ollama 并 `ollama pull qwen2.5:7b`、且 `ollama serve` 在跑；否则「AI 辅助」能进、但点「生成」会报错。AI 走本地模型，**数据不出本机**。
- **邮件**：演示环境若未配 SMTP，跳过「发送测试邮件」、口头说明即可；去重与双渠道逻辑不受影响。
- **保留数据**：演示机部署时**保留 `data/`**（尤其 `supervision.db`），不要覆盖内置演示库。
- **网络**：站内信本地即可；邮件 / AI 走本地或测试账号。
