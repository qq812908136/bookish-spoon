...
## 安全整改（见 `docs/上线前待办.md`）
- **DEF-001 / DEF-002 / DEF-004 代码侧 / DEF-005 全部已修复**（2026-08-31 ~ 09-01），代码侧四项缺陷清零。
- **DEF-002 CSRF**：`src/csrf.py` 签名令牌方案，`CSRF_ENABLED` 默认 `True`，覆盖 24 条写路由 / 21 个 POST 表单 / 8 处 AJAX。两层防护（Cookie `SameSite=Lax` + 令牌校验）互不依赖。
- **剩余只有部署侧**：目标机启用 HTTPS/域名/证书 + Waitress，按 `docs/生产部署指南.md` 执行（非代码默认值可决定）。
...
- 弱口令校验不追溯（仅新设/改密生效）：≥8 位、禁纯数字/纯字母、约 50 个黑名单、不得与用户名相同。
- **演示口令已于 2026-09-01 换成强口令**：管理员 `admin` / `Supv#Admin2026`，负责人 `zhangsan…zhengshi` / `Supv#Owner2026`（`seed_demo_data.py` + 初始化向导均已改）。exe 冒烟与演示统一用这套。
